...
void check_test_daq_link_frame(const pflib::packing::DAQLinkFrame& f, int bx,
                               int event, int orbit, bool corr3) {
...
BOOST_AUTO_TEST_CASE(real_full_frame) {
  ...
  BOOST_CHECK_MESSAGE((not ep.corruption[0]), "9b Header Marker same as Spec");
  BOOST_CHECK_MESSAGE((not ep.corruption[1]),
                      "Payload length in header equal to frame size minus 2");
  BOOST_CHECK_MESSAGE((not ep.corruption[2]),
                      "8b Header CRC matches transmitted value.");
  BOOST_CHECK_MESSAGE((not ep.corruption[3]),
  ...
  for (const auto& link : ep.links) {
    BOOST_CHECK_MESSAGE((not link.corruption[0]), "Link Stat checks all good");
    ...
    BOOST_CHECK_MESSAGE(link.calib.adc() == 0,
    ...
    for (const auto& ch : link.channels) {
      ...
      BOOST_CHECK_EQUAL(ch.tot(), -1);
      BOOST_CHECK_MESSAGE((ch.adc() > 0 and ch.adc() < 1024),
                          "channel ADC out of range");
      BOOST_CHECK_MESSAGE((ch.adc_tm1() > 0 and ch.adc_tm1() < 1024),
                          "channel ADC(t-1) out of range");
    }
  }
  /*
...
uint32_t econd_event_header_1(int length, bool passthrough) {
  return (0xaau << 24) + (length << 14) + (passthrough << 13) + (1u << 12) +
         (1u << 7);
}
...
BOOST_AUTO_TEST_CASE(econd_spec_example_fig33) {
  ...
  unsigned int l0_cm0{576}, l0_cm1{999}, l1_cm0{3}, l1_cm1{58};
  unsigned int ch0_adctm1{1}, ch0_adc{2}, ch1_adc{(1 << 6) + 1},
      ch2_adctm1{122}, ch2_adc{498}, ch3_adc{537}, ch3_toa{111},
  ...
  frame[4] =
      (ch0_adctm1 << 18) + (ch0_adc << 8) + (0b0001 << 4) + (ch1_adc >> 6);
  frame[5] = ((ch1_adc & mask<6>) << 26) + (0b0010 << 20) + (ch2_adctm1 << 10) +
             ch2_adc;
  frame[6] = (0b0011u << 28) + (ch3_adc << 18) + (ch3_toa << 8) + (0b10 << 6) +
             (ch4_adc >> 4);
  frame[7] = ((ch4_adc & mask<4>) << 28) + (ch4_tot << 18) + (ch4_toa << 8) +
             (0b11 << 6) + (ch5_adctm1 >> 4);
  frame[8] = ((ch5_adctm1 & mask<4>) << 28) + (ch5_tot << 18) + (ch5_toa << 8) +
             (0b01 << 6) + (ch6_adctm1 >> 4);
  ...
  BOOST_CHECK_MESSAGE(not ep.corruption[1],
                      "test frame and decoding have mismatched lengths");
  BOOST_CHECK_MESSAGE(
      not ep.corruption[2],
      "8-bit header CRC calculation done in software, needs to be updated once "
      "aligned with real hardware implementation");
  BOOST_CHECK_MESSAGE(not ep.corruption[3],
                      "Subpacket CRC calculation done in software, needs to be "
                      "updated once aligned with real hardware implementation");
  ...
  for (std::size_t i_ch{0}; i_ch < 36; i_ch++) {
    if (i_ch == 0 or i_ch == 10 or i_ch == 18 or i_ch == 26 or i_ch == 29 or
        i_ch == 31) {
      continue;
    }
...
BOOST_AUTO_TEST_CASE(one_link_passthrough) {
  ...
  frame[2] = eRx_header_1(false, ((frame[3] >> 10) & mask<10>),
                          (frame[3] & mask<10>)) +
             0b11111;
  frame[3] = 0xffffffff;
