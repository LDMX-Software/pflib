...
BOOST_AUTO_TEST_CASE(eg) {
  ...
  {
    auto tp = roc.testParameters().add("CH_40", "INPUTDAC", 0x2a).apply();
...
BOOST_AUTO_TEST_CASE(temp_zero) {
  ...
  {
    auto tp = roc.testParameters().add("CH_40", "INPUTDAC", 0).apply();
    auto test = roc.readPage(6, 1)[0];
    BOOST_CHECK_MESSAGE(test == 0, "test parameter set to zero");
  }
  auto test = roc.readPage(6, 1)[0];
  BOOST_CHECK_MESSAGE(test == 0x2a,
                      "parameter reset to original non-zero value");
