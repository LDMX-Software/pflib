...
int main(int argc, char** argv)
{
  ...
  std::string hardware_cfg,lpgbt_name,roc_config_file,pname;
  std::vector<std::string> rocs;
  try { 
    /** Define and parse the program options 
     */ 
    namespace po = boost::program_options; 
    po::options_description options("roc control options"); 
    options.add_options()
      ...
      ("rocs,r",               po::value<std::vector<std::string> >()->multitoken(), "names of the rocs you want to control")
      ("rocConfigFile,f",      po::value<std::string>(&roc_config_file), "path to configuration file use to read/write the roc")
      ("pname,p",              po::value<std::string>(&pname), "roc parammeter name to read or write, expected format : block.subblock.paramname (if a rocConfigFile is given, pname will be ignored)")
      ("value,v",              po::value<unsigned int>(&value), "value to write in roc parameter (ignored if write flag==0 and if the rocConfigFile is used)")
      ...
      ("configureI2CMaster,C", po::value<bool>(&configure_i2c_master)->default_value(false), "flag to set you want to reset and configure the I2C master before configuring/reading the ROC")
    ...
    try { 
      ...
      po::notify(vm);
      rocs = vm["rocs"].as<std::vector<std::string> >();
    }
    catch(po::error& e) { 
  ...
  if( roc_config_file.empty()==false )
    roc_config = YAML::LoadFile(roc_config_file);
  else{
    try{
      ...
      roc_config[block_name][subblock_name][param_name] = value;
    }
    catch( std::string& s ){
  ...
  for( auto roc_name : rocs ){
    // roc and lpgbt_i2c instances:
    YAML::Node roc_cfg = cfg["chip"][ roc_name ];
    auto i2c_name = roc_cfg["transport"].as<std::string>();
    YAML::Node i2c_cfg = cfg["transport"][ i2c_name ];   
    auto roc = chFactory.Create(roc_cfg["type"].as<std::string>(),roc_name,roc_cfg["cfg"]);  
    auto i2c = trFactory.Create(i2c_cfg["type"].as<std::string>(),i2c_name,i2c_cfg["cfg"]);
    roc->setTransport(i2c);
    ...
    if( read_write==1 ){//write
      roc->configure( roc_config );
    }
    else{
      auto read_config = roc->read( roc_config );
      //std::cout << read_config << std::endl;
      if( compare_and_alert ){
	auto expected = yaml_helper::compare(read_config,roc_config);
	if( expected )
	  spdlog::apply_all([&](LoggerPtr l) { l->info("{} : expected and read back configuration match",roc_name); });
	else
	  spdlog::apply_all([&](LoggerPtr l) { l->error("{} : expected and read back configuration DON'T match",roc_name); });
      }
      else
       std::cout << read_config << std::endl;
    }
