...
for ch_id, ch_df in chs:
    ...
    if (args.plot_individually):
        ...
        axi.set_ylabel('Median TOA [ns]')
        axi.set_xlabel('CALIB [a.u.]')
        axi.legend()
        axi.grid()
        figi.savefig(f'toa_time_walk_channel_{ch_id}.png', dpi=400)
        plt.close()

fig.supxlabel('CALIB [a.u.]')
ax0.set_ylabel('Median TOA [ns]')
ax0.set_title('Link 0')
...
fig.savefig("toa_time_walk.png", dpi=400)
