...
# Currently only works for one parameter given. todo
...
for column in samples:
    if column.find('CALIB') != -1:
        parameter_names.append(column)

samples['group'] = (samples[parameter_names[0]] != samples[parameter_names[0]].shift()).cumsum()

groups = list(samples.groupby('group'))
