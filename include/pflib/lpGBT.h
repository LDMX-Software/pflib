...
class lpGBT {
  ...
  */
  ...
  /** Check for PRBS errors on given eRx group and channel
      ...
      \param lpgbt_only Should we find errors with internal BERT (true) or external (false) like econ
      \param data_rate_code Data equal to 1, 2, or 3, from table 14.6 in v1 manual
      \param bert_time_code BERT measurement time from table 14.5 in v1 manual
  */
  void check_prbs_errors_erx(int group, int channel, bool lpgbt_only = false, int data_rate_code = 1, uint8_t bert_time_code = 4);
