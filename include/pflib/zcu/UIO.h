...
namespace pflib {
  ...
  public:
    ...
    uint32_t readMasked(size_t where, uint32_t mask) { return (read(where)&mask)>>first_bit_set(mask); }

    /** write with a mask (including shifts) */
    void writeMasked(size_t where, uint32_t mask, uint32_t val) { rmw(where,mask,(val<<first_bit_set(mask))&mask); }
