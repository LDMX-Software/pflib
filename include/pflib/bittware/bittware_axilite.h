...
/** This class provides access to read and write via the ioctl path to the AXILite targets within
    the Bittware firwmare.
    All addresses are full bus addresses relative to the base_address, including the two LSB which should always be zero.
*/
class AxiLite {
 public:
  AxiLite(const uint32_t base_address, const uint32_t mask_space=0x3FFFFF, const char* dev="/dev/datadev_0");
  ~AxiLite();
  ...
  bool isSet(uint32_t addr, int ibit) { return readMasked(addr,(1<<ibit))!=0; }
  /// set or clear a given bit
  void setclear(uint32_t addr, int ibit, bool true_for_set=false) { writeMasked(addr,(1<<ibit),(true_for_set)?(1):(0)); }
