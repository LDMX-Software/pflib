...
void align_phase_word(Target* tgt) {
  ...
  auto idle_0 =
      params.find("IDLEFRAME")->second;  // second because its a key value pair
                                         // mapping (See ROC.cxx)
  ...
  auto pusm_run = econ.dumpParameter(
      "CLOCKSANDRESETS",
      "GLOBAL_PUSM_STATE");  // dump Parameter I added to force this
                             // functionality to be able to assign to an output
                             // variable
  ...
  // Check phase_select
  auto phase_sel_0 = econ.dumpParameter(
  ...
  // .add("CHEPRXGRP_00", "CHANNEL_LOCKED", 1) // corresponding to
  // configs/check_erx_current_channel_locked_econ$ECON.yaml
  ...
  //   ROC ---------------------------------- // roc_setup_builder =
  //   roc.testParameters()
  //                            .add("DIGITALHALF_0", "IDLEFRAME", IDLE)
  //                            .add("DIGITALHALF_1", "IDLEFRAME", IDLE)
  //                           //  .add("TOP_0", "IN_INV_CMD_RX", INVERT_FCMD);
  //                           /// not needed
  //   roc_setup_test = roc_setup_builder.apply();
  //       // $BASE_CMD_ROC -l trg${SIDE} -r roc${IROC}_${SIDE}${MOD} -w 1 -p
  //       DigitalHalf.0.IdleFrame -v $IDLE
  //       //     $BASE_CMD_ROC -l trg${SIDE} -r roc${IROC}_${SIDE}${MOD} -w 1
  //       -p DigitalHalf.1.IdleFrame -v $IDLE
  ...
  //   ---------------------------------- // auto params =
  //   roc.getParameters("TOP_0"); // this uses the page and returns a mapping
  //   of all params therein auto inv_fc_rx =
