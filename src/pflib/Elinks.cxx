...
int Elinks::scanBitslip(int ilink) {
  ...
  for (islip = 0; islip < 32; islip++) {
    ...
    usleep(10);
    std::vector<uint32_t> sv = spy(ilink, true);
    //    printf("%d %d %08x\n",ilink,islip,sv[0]);
...
int Elinks::scanAlign(int ilink, bool debug) {
  ...
  for (int i = 0; i < 255; i += 5) {
    ...
    for (int test = 0; test < 100; test++) {
      std::vector<uint32_t> x = spy(ilink, true);
      if (test > 0 && x[0] != v) mismatches++;
      v = x[0];
    }
    if (debug) printf(" %d %d \n", i, mismatches);
    if (mismatches == 0 && zero_started < 0)
      zero_started = i;
  ...
  if (zero_started >= 0)
    points.push_back(std::pair<int, int>(zero_started, 255));
