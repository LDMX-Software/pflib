...
namespace pflib {
  namespace lpgbt {
    ...
    bool GPIO::getGPI(const std::string& name) {
      ...
      if (ptr==gpis_.end()) {
	PFEXCEPTION_RAISE("GPIOError", pflib::utility::string_format("Unknown GPI bit '%s'",name.c_str()));
      }
      int ibit=ptr->second;
    ...
    bool GPIO::getGPO(const std::string& name) {
      ...
      if (ptr==gpos_.end()) {
	// Print available names before raising the error
    ...
    void GPIO::setGPO(const std::string& name, bool toTrue) {
      ...
      if (ptr==gpos_.end()) {
	std::vector<std::string> names = getGPOs();
        printf("lpGBT set Available GPOs:\n");
        for (const auto& n : names) {
          printf("  %s\n", n.c_str());
        }
	PFEXCEPTION_RAISE("GPIOError", pflib::utility::string_format("Unknown GPO bit '%s'",name.c_str()));
	...
	if (cur->second==ibit || cur->first==name) cur=gpos_.erase(cur);
	else cur++;
      }
      for (auto cur = gpis_.begin(); cur!=gpis_.end(); ) {
	if (cur->second==ibit || cur->first==name) cur=gpis_.erase(cur);
	else cur++;
      }
      if (output) gpos_[name]=ibit;
      else gpis_[name]=ibit;
    }
