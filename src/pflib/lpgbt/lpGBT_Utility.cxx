...
lpGBT::RegisterValueVector loadRegisterCSV(const std::string& filename) {
  ...
  for (auto line : lines) {
    ...
    uint16_t addr;
    if (line.first.find("0x")==0 || line.first.find("0b")==0 || line.first.find_first_not_of("0123456789")==std::string::npos) {
      ...
      if (mask!=0xFF) {
        snprintf(mesg,120,"Invalid register for loadRegisterCSV, must be full register '%s'",line.first.c_str());
