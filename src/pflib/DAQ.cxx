...
DAQ::DAQ(WishboneInterface* wb) : wb_{wb} {
  n_links =
      (wb_->wb_read(tgt_DAQ_Control, CTL_CONTROL_REG) & CTL_LINK_COUNT_MASK) >>
...
void DAQ::setupLink(int ilink, bool zs, bool zs_all, int l1a_delay,
  ...
  regi = (regi & 0xFF) | ((l1a_delay & 0xFF) << 8) |
         ((l1a_capture_width & 0xFF) << 16);
...
void DAQ::getLinkSetup(int ilink, bool& zs, bool& zs_all, int& l1a_delay,
                       int& l1a_capture_width) {
  uint32_t reg1;
