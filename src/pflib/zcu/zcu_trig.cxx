...
void ZCUtrig::set_bx_delay(int ilink, int delay) {
  return uio_.writeMasked(ADDR_LINK_CAPTURE_DELAY, MASK_LINK_BX_DELAY_ZERO<<(BITS_OF_DELAY*ilink),delay&MASK_OF_DELAY);  
}

int ZCUtrig::get_bx_delay(int ilink) {
  if (ilink<0 || ilink>=nelinks_) return -1;
  return uio_.readMasked(ADDR_LINK_CAPTURE_DELAY, MASK_LINK_BX_DELAY_ZERO<<(BITS_OF_DELAY*ilink));
}

void ZCUtrig::setup_daq(int pipeline, int econ_id, int samples_per_l1a, int presamples) {
...
void ZCUtrig::get_daq_setup(int& pipeline, int& econ_id, int& samples_per_l1a, int& presamples) {
...
std::vector<uint32_t> ZCUtrig::read_event() {
  ...
  do {
    ...
    if (!(val&MASK_DAQ_TVALID)) {
      PFEXCEPTION_RAISE("ReadoutException","ZCU_TRIG got low TVALID before TLAST");
    }
    // get the next word
