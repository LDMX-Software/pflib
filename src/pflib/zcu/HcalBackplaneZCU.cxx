...
class HcalBackplaneZCU_Capture : public DAQ {
 ...
 public:
  ...
  virtual std::vector<uint32_t> getLinkData(int ilink) {
    ...
    capture_.writeMasked(ADDR_UPPER_ADDR, MASK_UPPER_ADDR, 0); // must be on basic page
...
class HcalBackplaneZCUTarget : public Target {
 public:
  ...
  virtual std::vector<uint32_t> read_event() override {
	...
	std::vector<uint32_t> subpacket=zcuhcal_->daq().getLinkData(0); // only one elink right now
	buf.push_back( (0x1<<28)|((zcuhcal_->daq().econid()&0xFFF)<<18)|(ievt<<13)|((ievt==zcuhcal_->daq().soi())?(1<<12):(0))|(subpacket.size()));
	...
	zcuhcal_->daq().advanceLinkReadPtr();
      ...
      PFEXCEPTION_RAISE("NoImpl",
			"HcalBackplaneZCUTarget::read_event not implemented for provided DaqFormat");
    }
