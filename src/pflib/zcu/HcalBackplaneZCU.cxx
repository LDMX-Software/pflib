 ...
 public:
  ...
  virtual int getEventOccupancy() {
    ...
    if (per_econ) 
      ...
      if (capture_.readMasked(ADDR_INFO,MASK_AXIS_NWORDS)!=0) return 1;
      else return 0;
  }
  ...
  virtual void enable(bool doenable) {
    if (doenable) capture_.rmw(ADDR_ENABLE,MASK_ENABLE,MASK_ENABLE);
    else capture_.rmw(ADDR_ENABLE,MASK_ENABLE,0);
    if (!per_econ_ && doenable) capture_.rmw(ADDR_PACKET_SETUP,AXIS_ENABLE,AXIS_ENABLE);
    else capture_.rmw(ADDR_PACKET_SETUP,AXIS_ENABLE,0);
  }
