...
std::vector<uint8_t> lpGBT_ICEC_Simple::read_regs(uint16_t reg, int n) {
  ...
  // wait for done...
  int timeout = 1000;
  for (val = transport_.read(offset_ + REG_STATUS_READ); (val & MASK_RX_EMPTY);
  ...
  while (!(val & MASK_RX_EMPTY)) {
    ...
    transport_.read(offset_ + REG_STATUS_READ);
    val = transport_.read(offset_ + REG_STATUS_READ);
  }
...
void lpGBT_ICEC_Simple::write_regs(uint16_t reg,
  ...
  while (wc < value.size()) {
    ...
    if (ic == 8 || wc == value.size()) {
      ...
      int timeout = 1000;
      for (uint32_t val = transport_.read(offset_ + REG_STATUS_READ);
           (val & MASK_TX_EMPTY);
           val = transport_.read(offset_ + REG_STATUS_READ)) {
        usleep(1);
        timeout--;
