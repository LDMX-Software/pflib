...
void delay_scan(Target* tgt) {
  ...
  for (int i = 1; i < 8; i++) {  // I hate this, but I have no better ideas atm
    for (int j = 1; j < 8; j++) {
      for (int k = 1; k < 8; k++) {
        auto globalanalog_page =
            pftool::utility::string_format("GLOBALANALOG_%d", link);
        delays = [ i, j, k ];
        delay_list.push_back(delays);
        inl_list.push_back(inl_scan(tgt, roc, n_events, channel_page,
                                    refvol_page, globalanalog_page, buffer,
                                    delays));
      }
    }
  }
