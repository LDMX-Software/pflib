...
namespace pflib::algorithm {
	...
	std::array<int, 2> tp50_scan(Target* tgt, ROC roc, std::array<int, 72> calib) {
		...
		std::array<int, 2> link_vref_list = {-1, -1}; // results array, which will hold the best vref for each link
		...
		for (int channel = 0; channel < 72; channel++) {
			...
			while (true) {
				...
				//pflib_log(info) << "Calculated efficiency " << tot_eff  << " for vref" << vref_value << ", channel " << channel;
				...
				if (std::abs(tot_eff - 0.5) < tol) {
					...
					if (link_vref_list[link] == -1) {
						...
						if (vref_buffer[index] <= link_vref_list[link]) {
							...
							link_vref_list[link] = vref_buffer[index];
							pflib_log(info) << "The chosen vref is not accounted for -> replacing vref in link";
							break;
						}
					}
				}
		...
		pflib_log(info) << "Finished finding vrefs for all channels." << '\n'
				<< "The vrefs per link are " << link_vref_list.front() << ", " << link_vref_list.back();
