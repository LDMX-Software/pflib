...
double eff_scan(Target* tgt, ROC roc, int channel, int vref_value, int nevents,
                auto refvol_page,
...
int global_vref_scan(Target* tgt, ROC roc, int channel, int nevents,
  ...
  while (true) {
    ...
    double efficiency =
        eff_scan(tgt, roc, channel, vref_value, nevents, refvol_page, buffer);
...
int local_vref_scan(Target* tgt, ROC roc, int channel, int vref_value,
                    int nevents, auto refvol_page, auto buffer) {
  ...
  for (int vref = vref_value; (vref > vref_value - 5) && (vref > 0); vref--) {
    double efficiency =
        eff_scan(tgt, roc, channel, vref, nevents, refvol_page, buffer);
  ...
  for (int vref = vref_value; (vref < vref_value + 5) && (vref < 600); vref++) {
    double efficiency =
        eff_scan(tgt, roc, channel, vref, nevents, refvol_page, buffer);
    auto diff = std::abs(efficiency - 0.5);
  ...
  int mindiff = *std::min_element(
      diff_buffer.begin(),
      diff_buffer.end());  // finding the efficiency closest to 0.5
  auto it = std::find(diff_buffer.begin(), diff_buffer.end(), mindiff);
  int index = -1;

  if (it !=
      diff_buffer.end()) {  // if it is not found, it assumes arr.end() (which
...
std::array<int, 2> tp50_scan(Target* tgt, ROC roc, std::array<int, 72> calib) {
  ...
  for (int channel = 0; channel < 72; channel++) {
    ...
    if (channel_eff < 0.5) {
      ...
      int channel_vref = local_vref_scan(tgt, roc, channel, vref, nevents,
                                         refvol_page, buffer);
      ...
      pflib_log(info)
          << "Channel not accounted for, initializing global vref scan...";
      int channel_vref =
          global_vref_scan(tgt, roc, channel, nevents, refvol_page, buffer);
      if (channel_vref != -1) {
