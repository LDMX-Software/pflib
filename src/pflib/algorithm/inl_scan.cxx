...
void linear_fit(const std::vector<double>& x,  // chatGPT code for now
                const std::vector<double>& y, double& m, double& b) {
  int n = x.size();
...
double inl(double& m, double b&, std::vector<double> calibs,
  ...
  auto vref_test_param =
      roc.testParameters()
          ...
          .add(channel_page, "LOWRANGE",
               1)    // should be preCC actually but idk how to set that up rn
