...
std::map<std::string, std::map<std::string, uint64_t>> t50_scan(
  ...
  // If a tot_vref is NULL, then we run a bisectional search for the 50 efficiency mark. 
  // If tot_vref is given, we raise the tot_vref if it is above 50% until it is below.
