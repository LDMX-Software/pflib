...
bool BWOptoLink::get_rx_polarity() {
  return (gtys_.readMasked(REG_POLARITY, MASK_POLARITY_RX) & (1 << (ilink_))) !=
         0;
}
bool BWOptoLink::get_tx_polarity() {
  return (gtys_.readMasked(REG_POLARITY, MASK_POLARITY_TX) & (1 << (ilink_))) !=
         0;
}
...
std::map<std::string, uint32_t> BWOptoLink::opto_rates() {
  ...
  */
  const char* tnames[] = {"S_AXI_ACLK", "LCLS2_CLK", "BX_CLK",
                          "TX_CLK",     "REF_CLK",   0};
  ...
  retval["RX-LINK"] = gtys_.read(REG_GTYS_RATES + 0x14 + 4 * ilink_) / 1000.0;
...
uint8_t BWlpGBT_Transport::read_reg(uint16_t reg) {
  ...
  if (retval.empty()) {
    PFEXCEPTION_RAISE("ReadFail", pflib::utility::string_format(
                                      "Failure to read register 0x%0x", reg));
  }
  return retval[0];
}
...
std::vector<uint8_t> BWlpGBT_Transport::read_regs(uint16_t reg, int n) {
  ...
  do {
    ...
    while (!(val & MASK_RX_EMPTY)) {
      ...
      val = transport_.readMasked(stsreg_, stsmask_);
      if (wc > 100) {
        snprintf(message, 256, "Read register 0x%x runaway (sts 0x%x)", reg,
                 val);
        PFEXCEPTION_RAISE("ICEC_Timeout", message);
      }
    }
...
void BWlpGBT_Transport::write_regs(uint16_t reg,
  ...
  int ic = 0;
  while (wc < value.size()) {
    transport_.write(ctloffset_ + REG_ADDR_TX_DATA,
                     baseval | uint32_t(value[wc]));
    wc++;
    ic++;
