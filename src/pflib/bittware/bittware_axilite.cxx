#include "pflib/bittware/bittware_axilite.h"

#include <stdlib.h>
#include <fcntl.h>
#include <errno.h>
#include <string.h>
#include "rogue/hardware/drivers/AxisDriver.h"
#include "pflib/Exception.h"
#include "pflib/utility/string_format.h"
#include <map>

namespace pflib {
namespace bittware {

static std::map<std::string,int> handle_map;

AxiLite::AxiLite(const uint32_t base_address, const uint32_t mask_space, const char* dev) : base_{base_address|0x00c00000}, mask_{mask_space&0xFFFFFFFCu}, antimask_{0xFFFFFFFFu^mask_} {
  auto ptr=handle_map.find(dev);
  if (ptr!=handle_map.end()) handle_=ptr->second;
  else {
    handle_ = open(dev, O_RDWR);
    if (handle_<0) {        
      PFEXCEPTION_RAISE("FileOpenException",pflib::utility::string_format("Error %s (%d) on opening '%s'",strerror(errno),errno,dev));
    }
    handle_map[dev]=handle_;
  }
}

AxiLite::~AxiLite() {
  // let the system close them...
}


uint32_t AxiLite::read(uint32_t addr) {
  if ((addr&antimask_)!=0) {
    PFEXCEPTION_RAISE("InvalidAddress",pflib::utility::string_format("Address 0x%0x is invalid",addr));
  }
  uint32_t val;
  dmaReadRegister(handle_,base_|addr,&val);
  return val;
}

void AxiLite::write(uint32_t addr, uint32_t val) {
  if ((addr&antimask_)!=0) {
    PFEXCEPTION_RAISE("InvalidAddress",pflib::utility::string_format("Address 0x%0x is invalid",addr));
  }
  dmaWriteRegister(handle_,base_|addr,val);
}

static  int first_bit_set(uint32_t mask) {                                                                                                                                                                          
  int i;                                                                                                                                                                                                       
  for (i = 0; i < 32; i++)                                                                                                                                                                                     
    if ((mask & (1 << i)) != 0) return i;                                                                                                                                                                      
  return 32;                                                                                                                                                                                                   
}  

uint32_t AxiLite::readMasked(uint32_t addr, uint32_t mask) {
  uint32_t val=read(addr);
  return (val&mask)>>(first_bit_set(mask));
}
  
void AxiLite::writeMasked(uint32_t addr, uint32_t mask, uint32_t nval) {
  uint32_t val=read(addr);
  val=(val&mask)|((nval<<first_bit_set(mask))&mask);
  write(addr,val);
}

}
}
