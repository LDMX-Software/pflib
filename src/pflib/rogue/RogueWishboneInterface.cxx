...
RogueWishboneInterface::RogueWishboneInterface(const std::string& host,
                                               int port) {
...
void RogueWishboneInterface::wb_write(int target, uint32_t addr,
                                      uint32_t data) {
  uint64_t fullAddr = (((uint64_t)(target)) << 32) | (addr);

  intf_->clearError();
  uint32_t id = intf_->reqTransaction(fullAddr, 4, &data,
...
uint32_t RogueWishboneInterface::wb_read(int target, uint32_t addr) {
  ...
  intf_->clearError();
  uint32_t id = intf_->reqTransaction(fullAddr, 4, &result,
...
void RogueWishboneInterface::wb_errors(uint32_t& crcup_errors,
...
void RogueWishboneInterface::fc_setup_calib(int pulse_len, int l1a_offset) {
  ...
  // remove old values
  reg = reg & (0xFFFFFFFFu ^
               (MASK_FC_SETUP_CALIB_PULSELEN << SHIFT_FC_SETUP_CALIB_PULSELEN));
  reg = reg & (0xFFFFFFFFu ^ (MASK_FC_SETUP_CALIB_L1AOFFSET
                              << SHIFT_FC_SETUP_CALIB_L1AOFFSET));
  // add new values
  reg = reg | ((pulse_len & MASK_FC_SETUP_CALIB_PULSELEN)
               << SHIFT_FC_SETUP_CALIB_PULSELEN);
  reg = reg | ((l1a_offset & MASK_FC_SETUP_CALIB_L1AOFFSET)
               << SHIFT_FC_SETUP_CALIB_L1AOFFSET);
  wb_write(TARGET_FC_BACKEND, REG_FC_SETUP, reg);
}
void RogueWishboneInterface::fc_get_setup_calib(int& pulse_len,
                                                int& l1a_offset) {
  uint32_t reg = wb_read(TARGET_FC_BACKEND, REG_FC_SETUP);
  pulse_len =
      (reg >> SHIFT_FC_SETUP_CALIB_PULSELEN) & MASK_FC_SETUP_CALIB_PULSELEN;
  l1a_offset =
      (reg >> SHIFT_FC_SETUP_CALIB_L1AOFFSET) & MASK_FC_SETUP_CALIB_L1AOFFSET;
}
void RogueWishboneInterface::fc_read_counters(int& spill_count, int& header_occ,
  ...
  vetoed = (wb_read(TARGET_FC_BACKEND, REG_FC_STATUS_VETOED) >> VETO_SHIFT) &
           VETO_MASK;
}
int RogueWishboneInterface::fc_timer_setup_read() {
  return (wb_read(TARGET_FC_BACKEND, REG_FC_PERIODIC_VETO) >>
          SHIFT_FC_PERIODIC) &
         MASK_FC_PERIODIC;
}
...
void RogueWishboneInterface::fc_enables_read(bool& ext_l1a, bool& ext_spill,
                                             bool& timer_l1a) {
...
void RogueWishboneInterface::fc_enables(bool ext_l1a, bool ext_spill,
                                        bool timer_l1a) {
  uint32_t reg = wb_read(TARGET_FC_BACKEND, REG_FC_CONTROL);
...
void RogueWishboneInterface::daq_status(bool& full, bool& empty, int& nevents,
                                        int& next_event_size) {
...
std::vector<uint32_t> RogueWishboneInterface::daq_read_event() {
  ...
  if ((ctl & MASK_DAQ_DMA_ENABLE) != 0) {
    ...
    PFEXCEPTION_RAISE("RogueWishboneInterface",
                      "DMA Readout is enabled, use daq_dma_run rather than "
                      "daq_read_event directly.");
  } else {
    // non-dma readout
    uint32_t status = wb_read(TARGET_DAQ_BACKEND, REG_DAQ_STATUS);
    ...
    uint64_t fullAddr =
        (((uint64_t)(TARGET_DAQ_BACKEND)) << 32) | (BASE_DAQ_BUFFER);

    intf_->clearError();
    uint32_t id = intf_->reqTransaction(fullAddr, next_event_size * 4, &(x[0]),
                                        ::rogue::interfaces::memory::Read);
    intf_->waitTransaction(id);
    if (intf_->getError() != "") {
      PFEXCEPTION_RAISE("RogueWishboneInterface", intf_->getError().c_str());
...
void RogueWishboneInterface::daq_dma_setup(uint8_t fpga_id,
  ...
  ctl |=
      (samples_per_event << SHIFT_SAMPLES_PER_EVENT) & MASK_SAMPLES_PER_EVENT;
  wb_write(TARGET_DAQ_BACKEND, REG_DAQ_SETUP, ctl);
}
void RogueWishboneInterface::daq_get_dma_setup(uint8_t& fpga_id,
                                               uint8_t& samples_per_event,
                                               bool& enabled) {
  uint32_t ctl = wb_read(TARGET_DAQ_BACKEND, REG_DAQ_SETUP);
...
void RogueWishboneInterface::daq_setup_event_tag(int run, int day, int month,
                                                 int hour, int min) {
  uint32_t tag = (run & 0xFFF) | ((min & 0x3F) << 12) | ((hour & 0x1F) << 18) |
                 ((day & 0x1F) << 23) | ((month & 0xF) << 28);
...
void RogueWishboneInterface::daq_dma_dest(
    std::shared_ptr<::rogue::interfaces::stream::Slave> sl) {
  dma_client_->addSlave(sl);
}

void RogueWishboneInterface::daq_dma_run(const std::string& cmd, int run,
                                         int nevents, int rate) {
  time_t t = time(NULL);
  struct tm* gmtm = gmtime(&t);
  this->daq_setup_event_tag(run, gmtm->tm_mday, gmtm->tm_mon + 1, gmtm->tm_hour,
                            gmtm->tm_min);
  ...
  for (int i{0}; i < nevents; i++) {
    if (cmd == "PEDESTAL") fc_sendL1A();
    if (cmd == "CHARGE") fc_calibpulse();
...
void RogueWishboneInterface::daq_dma_close() { dma_dest_->close(); }
