...
void ECONTCaptureHeader::from(std::span<uint32_t> data) {
  if (data.size() < 2) {
    pflib_log(error) << "ECON-T capture packet header requires 2 words but recieved " << data.size();
    return;
  }
  /**
...
int ECONTCaptureHeader::version() const {
  return version_;
}

int ECONTCaptureHeader::econ_id() const {
  return econ_id_;
}

int ECONTCaptureHeader::length() const {
  return length_;
}

int ECONTCaptureHeader::n_links() const {
  return n_links_;
}

int ECONTCaptureHeader::pre_samples() const {
  return pre_samples_;
}

int ECONTCaptureHeader::n_samples() const {
  return n_samples_;
}
}
