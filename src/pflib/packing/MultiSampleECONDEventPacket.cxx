...
void MultiSampleECONDEventPacket::from(std::span<uint32_t> data) {
  ...
  ievent = (data[1] >> 8);
  bx = (data[1] & mask<8>);
  pflib_log(trace) << hex(data[1]) << " -> ievent, bx = " << ievent << ", " << bx;
  ...
  pflib_log(trace) << hex(data[3]) << " -> contrib, subsys = " << contrib_id << ", " << subsys_id;
  ...
  while (offset < data.size()) {
    ...
    pflib_log(trace) << hex(data[offset]) << " -> link_len, il1a, econ_id, is_soi, is_last = "
      << link_len << ", " << il1a << ", " << econ_id << ", " << is_soi << ", " << is_last;

    if (i_sample != il1a) {
      pflib_log(warn) << "mismatch between transmitted index and unpacking index for sample "
        << i_sample << " != " << il1a;
    }
    ...
    if (is_last) {
      pflib_log(trace) << "Last sample packet found, leaving loop at offset = " << offset
        << " (data.size() = " << data.size() << ")";
      break;
    }
