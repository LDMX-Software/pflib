...
SingleECONDRocErxMapping::SingleECONDRocErxMapping(
  ...
  for (int i_roc{0}; i_roc < roc_half_to_erx_.size(); i_roc++) {
    ...
    if (std::find(active_rocs.begin(), active_rocs.end(), i_roc) != active_rocs.end()) {
...
std::pair<int, int> SingleECONDRocErxMapping::toROCChannel(int i_erx, int channel) const {
  if (channel < 0 or channel >= 36) {
    PFEXCEPTION_RAISE("BadChannel",
        "You gave an invalid channel with the eRx index ("+std::to_string(channel)+") "
...
int SingleECONDRocErxMapping::toErx(int i_roc, int half) const {
  if (half != 0 and half != 1) {
    PFEXCEPTION_RAISE("BadHalf", "A ROC's half is either 0 (lower channels) or 1 (upper channels).");
  }
  const auto& [half_0_eRx, half_1_eRx] = roc_half_to_erx_.at(i_roc);
  return erx_to_i_erx_.at((half == 0) ? half_0_eRx : half_1_eRx);
}

std::pair<int, int> SingleECONDRocErxMapping::toErxChannel(int i_roc, int channel) const {
  if (channel < 0 or channel >= 72) {
    PFEXCEPTION_RAISE("BadChannel",
        "You gave an invalid channel with the ROC index ("+std::to_string(channel)+") "
