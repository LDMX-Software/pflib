...
void FastControl::getErrorCounters(uint32_t& single_bit_errors, uint32_t& double_bit_errors) {
...
void FastControl::resetCounters() {
  wb_write(REG_CONTROL1,CTL_COUNTER_CLEAR);
}
...
void FastControl::setupMultisample(bool enable, int extra_samples) {
  wb_rmw(REG_CONTROL2,extra_samples<<CTL_MULTISAMPLE_SHIFT,CTL_MULTISAMPLE_MASK);
  if (enable) {
