...
int lpGBT::find_vref_tune(double vref_slope, double vref_offset, double temp_uncal_slope, double temp_uncal_offset) {
	write(REG_VREFTUNE, 0);
	double adc_avg = adc_average();
	// printf("  > adc_avg = %f\n", adc_avg);
        double tj_user = adc_avg * temp_uncal_slope + temp_uncal_offset;
	// printf("  > tj_user = %f\n", tj_user);
	int optimal_vref_tune = static_cast<int>(std::round(tj_user * vref_slope + vref_offset));
...
void lpGBT::read_internal_temp_avg() {
  ...
  double tj_user = adc_avg * AVG_TEMPERATURE_UNCALVREF_SLOPE + AVG_TEMPERATURE_UNCALVREF_OFFSET;
  printf("  > Esitimated junction temperature (TJ_USER): %f degrees C\n",
         tj_user);

  double vadc_v = adc_avg * (AVG_ADC_X2_SLOPE + tj_user * AVG_ADC_X2_SLOPE_TEMP) + AVG_ADC_X2_OFFSET + tj_user * AVG_ADC_X2_OFFSET_TEMP;
  printf("  > Calibrated Voltage: %.4f V\n", vadc_v);

  double temperature_c = (vadc_v * AVG_TEMPERATURE_SLOPE) + AVG_TEMPERATURE_OFFSET;
  printf("  > ----------------------------------------------------\n");
...
void lpGBT::read_internal_temp_precise(YAML::Node cal_data) {
  ...
  double TEMP_UNCALVREF_SLOPE = cal_data["TEMPERATURE_UNCALVREF_SLOPE"].as<double>();
  double TEMP_UNCALVREF_OFFSET = cal_data["TEMPERATURE_UNCALVREF_OFFSET"].as<double>();
 
  write(REG_ADCMON, 1 << 5);
  usleep(1000);
  write(REG_ADCMON, 0 << 5);

  int vref_tune = find_vref_tune(VREF_SLOPE, VREF_OFFSET, TEMP_UNCALVREF_SLOPE, TEMP_UNCALVREF_OFFSET);
