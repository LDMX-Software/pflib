...
class TargetFiberless : public Target {
 ...
 private:
  ...
  void TargetFiberless::setup_run(int run, int format, int contrib_id) {
    run_=run;
    daqformat_=format;
    if (contrib_id<0) contribid_=255;
    else contribid_=contrib_id&0xFF;
    ievt_=0;
    l1a_=0;
    fc_->clear_run();
  }
...
std::vector<uint32_t> TargetFiberless::read_event() {
  ...
  if (has_event()) {
	...
	if (i%2) buffer[2+1+i/2]|=(len<<16);
      ...
      buffer.push_back(0);
      buffer.push_back((0xA6u<<24)|(contribid_<<16)|(SUBSYSTEM_ID_HCAL_DAQ<<8)|(0));
      
      for (int il1a=0; il1a<hcal().daq().samples_per_ror(); il1a++) {
	formatter_.startEvent(bc+il1a*2,l1a_+il1a,0); // assume orbit zero, L1A spaced by two
	for (int i=0; i<2; i++) { // only the DAQ links here
	  formatter_.add_elink_packet(i,hcal().daq().getLinkData(i));
	}
	formatter_.finishEvent();
	uint32_t header=formatter_.getPacket().size()+1; // include the header
	header|=(0x1<<28);
	...
	buffer.insert(buffer.end(),formatter_.getPacket().begin(),formatter_.getPacket().end());	
