...
uhalWishboneInterface::uhalWishboneInterface(
    const std::string& target, const std::string& ipbus_map_path) {
  std::string addressTable_uMNio("file://");
  addressTable_uMNio += ipbus_map_path;
  if (not ipbus_map_path.empty() and ipbus_map_path.back() != '/')
    addressTable_uMNio += '/';
  ...
  hw_ = std::make_unique<::uhal::HwInterface>(
      ::uhal::ConnectionManager::getDevice(target, location,
                                           addressTable_uMNio));
}
...
void uhalWishboneInterface::fc_get_setup_calib(int& pulse_len,
                                               int& l1a_offset) {
  const ::uhal::Node& m_base(hw_->getNode("LDMX"));
  ::uhal::ValWord<uint32_t> pl =
      m_base.getNode("FAST_CONTROL.CALIB_PULSE_LEN").read();
  ::uhal::ValWord<uint32_t> lo =
      m_base.getNode("FAST_CONTROL.CALIB_L1A_OFFSET").read();
  dispatch();
  pulse_len = pl;
  l1a_offset = lo;
}
...
void uhalWishboneInterface::daq_status(bool& full, bool& empty, int& nevents,
                                       int& next_event_size) {
  const ::uhal::Node& m_base(hw_->getNode("LDMX"));
