...
void align_phase_word(Target* tgt) {
  ...
  auto pusm_run = econ.dumpParameter(
      "CLOCKSANDRESETS",
      "GLOBAL_PUSM_RUN");  // dump Parameter I added to force this functionality
                           // to be able to assign to an output variable
  ...
  if (pusm_state == 8) {
    ...
    {  // scope this
      ...
      auto params = roc.getParameters(
          "DIGITALHALF_0");  // this uses the page and returns a mapping of all
                             // params therein
      auto idle_0 =
    ...
    //   auto econ_setup_builder = econ.testParameters().add("CLOCKSANDRESETS",
    //   "GLOBAL_PUSM_RUN", 0); // set run bit 0 while configuring auto
    //   econ_setup_test = econ_setup_builder.apply(); auto pusm_run =
    //   econ.dumpParameter("CLOCKSANDRESETS","GLOBAL_PUSM_RUN");   // dump
    //   Parameter I added to force this functionality to be able to assign to
    //   an output variable auto pusm_state =
    //   econ.dumpParameter("CLOCKSANDRESETS","GLOBAL_PUSM_STATE"); std::cout <<
    //   "First PUSM_STATE = " << pusm_state << std::endl ; std::cout << "First
    //   PUSM_RUN = " << pusm_run << std::endl ;
    // }
    ...
    {  // scope this
      auto econ_setup_builder =
          econ.testParameters()
              .add("EPRXGRPTOP", "GLOBAL_TRACK_MODE",
                   1)  // corresponding to configs/train_erx_phase_ON_econ.yaml
              .add("CHEPRXGRP", "0_TRAIN_CHANNEL",
                   1)  // corresponding to
                       // configs/train_erx_phase_TRAIN_econ.yaml
    ...
    //   auto econ_setup_test = econ_setup_builder.apply();
    //   auto pusm_run =
    //   econ.dumpParameter("CLOCKSANDRESETS","GLOBAL_PUSM_RUN"); auto
    //   pusm_state = econ.dumpParameter("CLOCKSANDRESETS","GLOBAL_PUSM_STATE");
    //   std::cout << "PUSM_STATE = " << pusm_state << std::endl ;
    //   std::cout << "PUSM_RUN = " << pusm_run << std::endl ;
    // }
    ...
    {  // scope this
      ...
      auto econ_setup_builder =
          econ.testParameters()
              .add("CHEPRXGRP", "0_TRAIN_CHANNEL",
                   0)  // corresponding to configs/train_erx_phase_OFF_econ.yaml
    ...
    {  // scope this
      ...
      auto econ_setup_builder =
          econ.testParameters()
              .add(
                  "CHEPRXGRP", "0_CHANNEL_LOCKED",
                  1)  // corresponding to
                      // configs/check_erx_current_channel_locked_econ$ECON.yaml
      ...
      std::cout << "CHEPRXGRP5 (Example lock_channel 5) = " << cheprxgrp5
                << std::endl;
    };
    // --------------------------------------------------------------------------------------------
    // //
    ...
    // Check channel lock
    auto ch_lock_0 = econ.dumpParameter(
        "CHEPRXGRP", "0_CHANNEL_LOCKED");  // can also readParameters()
    ...
    {  // scope this
      auto params =
          roc.getParameters("TOP");  // this uses the page and returns a mapping
                                     // of all params therein
    ...
    // LINK_RESET
    //  tgt->fc().linkreset_rocs();     // is this sufficient? Do I need to set
    //  a value e.g. ./uhal_backend_v3.py -b
    //  Housekeeping-FastCommands-fastcontrol-axi-0 --node
    //  bx_link_reset_roc${ECON} --val 3516
