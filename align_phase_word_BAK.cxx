...
void align_phase_word(Target* tgt) {
  ...
  auto econ =
      tgt->hcal().econ(pftool::state.iecon);  //, pftool::state.type_version());
  ...
  // ---------------------------------- // Is this how I set ROC parameters
  // appropriately?
  ...
  {  // scope this
    auto econ_setup_builder =
        econ.testParameters()
            ...
            .add("CHEPRXGRP", "0_TRAIN_CHANNEL",
                 1)  // corresponding to configs/train_erx_phase_TRAIN_econ.yaml
    ...
    std::cout << "CHEPRXGRP5 = " << cheprxgrp5
              << std::endl;  // arbitrarily using 5
  }

  {  // scope this
    // Set run bit ON
    auto econ_setup_builder =
        econ.testParameters().add("CLOCKSANDRESETS", "GLOBAL_PUSM_RUN", 1);
    auto econ_setup_test = econ_setup_builder.apply();
  ...
  {  // scope this
    // Set run bit OFF
    auto econ_setup_builder =
        econ.testParameters().add("CLOCKSANDRESETS", "GLOBAL_PUSM_RUN", 0);
    auto econ_setup_test = econ_setup_builder.apply();
    auto pusm_run = econ.dumpParameter("CLOCKSANDRESETS", "GLOBAL_PUSM_RUN");
    auto pusm_state =
        econ.dumpParameter("CLOCKSANDRESETS", "GLOBAL_PUSM_STATE");
  ...
  {  // scope this
    ...
    auto econ_setup_builder =
        econ.testParameters()
            .add("CHEPRXGRP", "0_CHANNEL_LOCKED",
                 1)  // corresponding to
  ...
  //   // ---------------------------------- SETTING ROC REGISTERS to configure
  //   ROC ---------------------------------- //
  {  // scope this
    auto roc_setup_builder = roc.testParameters()
                                 .add("DIGITALHALF_0", "IDLEFRAME", IDLE)
  ...
  //   ---------------------------------- //
  {                                          // scope this
    auto params = roc.getParameters("TOP");  // this uses the page and returns a
                                             // mapping of all params therein
    auto inv_fc_rx =
        params.find("IN_INV_CMD_RX")->second;  // second because its a key value
                                               // pair mapping (See ROC.cxx)
  ...
  {  // scope this
    auto econ_setup_builder = econ.testParameters().add(
        "ALIGNER", "GLOBAL_ORBSYN_CNT_SNAPSHOT",
        0);  // UNSURE what this parameter should be. "snapshot location BX" or
             // "ORBSYNCSNAPSHOT"
    auto econ_setup_test = econ_setup_builder.apply();
  }
  //   // ----------------------------------
  ...
  // DO I need a second round of alignment settings?     //Configure ECOND for
  // Alignment (from econd_init_cpp.yaml)
  ...
  // LINK_RESET
  tgt->fc().linkreset_rocs();  // is this sufficient? Do I need to set a value
                               // e.g. ./uhal_backend_v3.py -b
                               // Housekeeping-FastCommands-fastcontrol-axi-0
                               // --node bx_link_reset_roc${ECON} --val 3516
