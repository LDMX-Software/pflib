...
std::tuple<double, double> linear_regression(
  ...
  if (x_vals.size() != y_vals.size()) {
    throw std::invalid_argument("x_vals and y_vals must be the same size."); //applies a sanity check to see if the data that it is analyzing is of a suitable size
  }
...
std::string save_name = pftool::readline("Name of file that you would like to save TRIM_TOA efficiency data (empty if you do not want the file):");
  ...
  tgt->fc().fc_setup_calib(17); //Sets the bunch crossing value, should have the optimal value be the same for both links on a ROC, may want to change
...
for (int i_link{0}; i_link < 2; i_link++) {
      ...
      charge_active[refvol_page]["CHOICE_CINJ"] = 1; //needed to properly apply calib vals, but should stay on for the entire scan
      }
...
----CALIB
------Data collection
--------Data for the target channel on each ROC simultaneously, from the one charge injection run
----Stopping the CALIB loop for a given Trim_toa value once it had reached a point where each ROC has had an efficiency of <0.9 at least once
...
*/
  ...
  for (int ch{0}; ch < 72; ch++) {
    ...
    param_ch[ch_str]["HIGHRANGE"] = 1; //set in the highrange, if you want to change to the low range you need to change the tgt-calib value used later in the calculation, and change Calib to loop
    param_ch[ch_str]["LOWRANGE"] = 0;  //from 0 to 1000, and most likely have a higher range of trim_toa values, but the code should still run the same otherwise.
    ...
    for (int trim_toa{0}; trim_toa < trim_toa_max; trim_toa += trim_toa_step) {
      ...
      auto all_good = [&]() {
        for (const auto& [i_roc, val] : good_trim_val) {
           if (!val) return false; //condition to stop scanning for a given calib range, needs each ROC to have at least one good toa_efficiency value
        }
          return true;
        };

      std::map<std::string, std::map<std::string, uint64_t>> trim;
      trim[ch_str]["TRIM_TOA"] = trim_toa; //apply trim_toa to each ROC on the specified channel
      ...
      for (int calib = calib_min; calib < calib_max && !all_good(); calib += calib_step ) {
        ...
        for (std::size_t i = 0; i < length; i++){ //find number of times TOA triggered during the run
        ...
        double percent = ((toa_count * 1.0) / length); //make it into % efficiency value
      // need to divide by calib_step and trim_toa_step because index is value/4 from final_data, initialization and for loop
        ...
        if (percent > 0.95) {
          ...
          good_trim_val[i_roc] = true;                                                                                                           //should show if your toa efficiency seems reasonable
       }
      }
     }
    }
...
pflib_log(info) << "Saved data file successfully";
   ...
   * point where toa_efficiency goes reaches an efficiency of >0.9. The toa_efficiency
  ...
  for (int trim_toa{0}; trim_toa < trim_toa_max; trim_toa += trim_toa_step) {
    for (int ch{0}; ch < 72; ch++) {
      for (int calib{calib_min}; calib < calib_max ; calib += calib_step) {
        ...
        if (final_data[roc_index[i_roc]][(calib - calib_min) / calib_step][trim_toa / trim_toa_step][ch]> 0.9) { //if your scan can't meet this efficiency value then you are probably not applying the settings well or you have a hardware issue
...
int calib_tgt = 75 ;  //base this value around what your target calib value is for the highrange, MUST BE CHANGED IF SCAN IS RAN WITH LOWRANGE ENABLED
  ...
  for (int ch{0}; ch < 72; ch++) {
    ...
    for (const auto& row : threshold_points) {
      ...
      if (row[0] == ch) {
        ...
        trim_toa.push_back(row[2]); //append the first trim_toa and CALIB value that met this toa efficiency threshold
       }
      }
    }
    ...
    int optimal_trim_val_clam = std::clamp(optimal_trim_val_round, 0, 63); //need to keep the data in this range to allow it to be applied
...
for (int ch = 0; ch < 72; ch++) { //calculate and then apply this average value to all of the channels that are not producing good data
    if (target[roc_index[i_roc]][ch] == -1) { //shouldnt matter as TOA isn't firing for them
...
std::map<int, std::map<std::string, std::map<std::string, uint64_t>>> settings; //make map that is then applied to the correct channel and ROC
 for (int i_roc : tgt-> roc_ids()) {
  for (int ch{0}; ch < 72; ch++) {
