...
std::map<int, std::map<std::string, std::map<std::string, uint64_t>>> toa_scan(
  ...
  int target_calib = pftool::readline_int("Target calib?", 1200);
  int vref_buffer = pftool::readline_int("Vref buffer?", 5);
      ...
      for (int ch{0}; ch < 72; ch++) {
        ...
        for (std::size_t i{0}; i < data.size(); i++) {
          ...
          if ((toa > 0) && (vrefs[i_roc][ch / 36] == -1)) {
            ...
            pflib_log(info) << "Roc: " << i_roc << " link: " << ch / 36
                            << " vref: " << vrefs[i_roc][ch / 36];
          }
        }
      }
    }
  ...
  // increases target vref until half of channels are below 0.5 eff at trim_toa
  // = 32
  while (!complete) {
    std::map<int, std::map<std::string, std::map<std::string, uint64_t>>>
        buffer_vrefs;
    ...
    for (int i_roc : tgt->roc_ids()) {
      ...
      for (int i_link{0}; i_link < 2; i_link++) {
        // pflib_log(info) << "link: " << i_link << " vref: " <<
        // vrefs[i_roc][i_link] << " low: " << low_ch[i_link];
        if ((low_ch[i_link] >= 18) && (vref_targets[i_roc][i_link] == -1)) {
          vref_targets[i_roc][i_link] = vrefs[i_roc][i_link];
          pflib_log(info) << "Roc: " << i_roc << " link: " << i_link
                          << " vref: " << vref_targets[i_roc][i_link];
  ...
  for (int i_roc : tgt->roc_ids()) {
    for (int i_link{0}; i_link < 2; i_link++) {
      ...
      link_parameters[i_roc][refvol_page]["TOA_VREF"] =
  ...
  for (int trim_toa{0}; trim_toa < 64; trim_toa++) {
    ...
    std::map<std::string, std::map<std::string, uint64_t>> toa_parameters;
    for (int ch{0}; ch < 72; ch++) {
      auto ch_page = pflib::utility::string_format("CH_%d", ch);
