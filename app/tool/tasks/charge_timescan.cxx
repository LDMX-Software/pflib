...
static void charge_timescan_runs(Target* tgt, ROC& roc, size_t nevents, bool isLED, int channel, int link, int i_ch, std::string fname, int calib, bool highrange, int start_bx, int n_bx){
    ...
    [&](std::ofstream& f,
      ...
      if constexpr (std::is_same_v<EventPacket,pflib::packing::MultiSampleECONDEventPacket>){
        ...
        static_assert(sizeof(EventPacket) == 0, "BadConf",
                        "Unable to do all_channels_to_csv for the currently configured format.");
      }      
      f << '\n';
    }};
...
void charge_timescan(Target* tgt) {
  ...
  if (pftool::state.daq_format_mode == Target::DaqFormat::SIMPLEROC) {
    charge_timescan_runs<pflib::packing::SingleROCEventPacket>(tgt, roc, nevents, isLED, channel, link, i_ch, fname, calib, highrange, start_bx, n_bx);
  } else if (pftool::state.daq_format_mode == Target::DaqFormat::ECOND_SW_HEADERS) {
    charge_timescan_runs<pflib::packing::MultiSampleECONDEventPacket>(tgt, roc, nevents, isLED, channel, link, i_ch, fname, calib, highrange, start_bx, n_bx);
  }
