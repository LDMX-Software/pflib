...
void align_phase_word(Target* tgt) {
  ...
  // ----- PHASE ALIGNMENT SCOPE ----- //
  {
    ...
    int invertfcmd = 1;
    parameters = {
        {"FCTRL", {{"GLOBAL_INVERT_COMMAND_RX", invertfcmd}}}
    };
    ...
    auto pusm_state = econ.dumpParameter(
      "CLOCKSANDRESETS", "GLOBAL_PUSM_STATE");
  ...
  // ------------ WORD ALIGNMENT scope ----------- //
  {
    ...
    // ---- RE SETTING ROC REGISTERS ---- //
      ...
      std::cout 
        << "bxtrigger_0 = " << bxtrig_0 
        << ", 0x" << std::hex << bxtrig_0 
        << std::dec 
        << std::endl;
      std::cout 
        << "bxtrigger_1 = " << bxtrig_1 
        << ", 0x" << std::hex << bxtrig_1 
        << std::dec 
        << std::endl;
    // ---------------------------------- //
    ...
    // ---- SETTING ECON REGISTERS ---- //
      ...
      for (int channel : list_channels) {
        ...
        std::string var_name_erx = 
          std::to_string(channel) + "_ENABLE";
        std::string var_name_erxINV = 
          std::to_string(channel) + "_INVERT_DATA";
    ...
    //   ----- READING ECON REGISTERS ----- //
      ...
      for (int channel : list_channels) {
        ...
        std::cout 
          << "GLOBAL_MATCH_PATTERN_VAL test: "
          << global_match_pattern_val 
          << ", 0x" << std::hex << global_match_pattern_val 
          << std::dec 
          << std::endl;
      }

      auto cnt_load_val = econ.dumpParameter(
        "ALIGNER", 
        "GLOBAL_ORBSYN_CNT_LOAD_VAL");
      auto snapshot_arm = econ.dumpParameter(
        "ALIGNER", 
        "GLOBAL_SNAPSHOT_ARM");
    ...
    // ----------------------------------- //
      ...
      for (int channel : list_channels) {
        std::string var_name_align =
          std::to_string(channel) + "_PER_CH_ALIGN_EN";
        std::string var_name_pm = 
          std::to_string(channel) + "_PATTERN_MATCH";
        ...
        std::string var_name_select = 
          std::to_string(channel) + "_SELECT";
        ...
        std::cout 
          << "channel_snapshot1 " << channel << " = " << ch_snapshot_1
          << ", 0x" << std::hex << ch_snapshot_1 
          << std::dec
          << std::endl;
        std::cout 
          << "channel_snapshot2 " << channel << " = " << ch_snapshot_2
          << ", 0x" << std::hex << ch_snapshot_2 
          << std::dec
          << std::endl;
        std::cout 
          << "channel_snapshot3 " << channel << " = " << ch_snapshot_3
          << ", 0x" << std::hex << ch_snapshot_3 
          << std::dec
          << std::endl;
        std::cout 
          << "channel_snapshot_full_hexstring " << channel 
          << " = 0x" 
          ...
          << std::endl;
        std::cout 
          << "channel_snap " << channel << " = " << ch_snap 
          << ", 0x" << std::hex << ch_snap
          << std::dec 
          << std::endl;
        std::cout 
          << "chAligner pattern_match = " << ch_pm 
          << ", 0x" << std::hex << ch_pm 
          << std::dec 
          << std::endl;
        std::cout 
          << "chAligner snapshot_dv = " << ch_snap_dv 
          << ", 0x" << std::hex << ch_snap_dv 
          << std::dec 
          << std::endl;
        std::cout 
          << "chAligner select " << channel << " = " << ch_select
          << ", 0x" << std::hex << ch_select 
          << std::dec
          << "(0xa0 = failed alignment)" 
          << std::endl;
      }
    } else {
      std::cout << "PUSM_STATE / runbit does not equal 8. Not running phase "
