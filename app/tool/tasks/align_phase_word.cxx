...
void align_phase_word(Target* tgt) {
  ...
  {  // set inversion value - try a second call to test parameters to force this
     // inversion parameter to get written (so I dont have to run the whole
     // thing twice)
    auto econ_setup_builder = econ.testParameters().add(
        "FCTRL", "GLOBAL_INVERT_COMMAND_RX",
        1);  // set fctrl inversion so that ECON can Lock.
  ...
  if (pusm_state == 8) {
    ...
    for (int i = 0; i < std::size(list_channels); i++) {
      binary_channels =
          binary_channels |
          (1 << list_channels[i]);  // bit wise OR comparison between e.g. 6 and
                                    // 7, and shifting the '1' bit in the lowest
                                    // sig bit, with << operator by the amount
                                    // of the channel #.
    }
    ...
    {  // scope
      // Check channel lock
      std::map<int, int>
          ch_lock_values;  // create map for storing channel - value. Note this
                           // is not used, but here just in case its needed.
      for (int channel : list_channels) {
