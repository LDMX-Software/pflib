...
ENABLE_LOGGING();void channel_wise_calib_scan(Target* tgt) {
  ...
  auto test_setup_params = tgt->tempApplyAllROCs(setup_parameters);  int central_charge_to_l1a;
  int charge_to_l1a{0};
  int phase_strobe{0};
  double time{0};
  ...
  int calib{0};
  int ch{0};  std::map<int, std::string> fnames;
  ...
  for (int i_roc : tgt->roc_ids()) {
    ...
    n_links
  };  tgt->setup_run(1, Target::DaqFormat::ECOND_SW_HEADERS, 1);  central_charge_to_l1a = tgt->fc().fc_get_setup_calib();
  for (ch = min_ch; ch < max_ch + 1; ch++) {
    ...
    for (calib = min_calib; calib < max_calib; calib += stepsize) {
      ...
      auto test_params = tgt->tempApplyAllROCs(parameters);      for (charge_to_l1a = central_charge_to_l1a + start_bx;
