...
class AD5593R {
 ...
 public:
  ...
  bool is_gpo(int pin) { return read_reg(REG_GPO_ENABLE) & (1 << pin); }

  bool gpo_get_value(int pin) { return read_reg(REG_GPO_WRITE) & (1 << pin); }
  ...
  float adc_volts(int pin, int ave = 10) {
    int sum = 0;
    for (int i = 0; i < ave; i++) sum += adc_read(pin);
